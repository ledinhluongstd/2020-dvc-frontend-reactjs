...
class DanhSach extends Component {
  ...
  componentDidMount = async () => {
    this._getDanhSachBieuMau(this._createFilter())
  }
  ...
  _getDanhSachBieuMau = async (query) => {
    let data = await tbEform.getAll(query)
  ...
  _handleDeleteMulti = async () => {
    ...
    danhsach.forEach((item, index) => {
      if (item.checked)
        axiosReq.push(tbEform.deleteById(item._id.$oid || item._id))
    });
  ...
  _handleLockMulti = async () => {
    ...
    danhsach.forEach((item, index) => {
      if (item.checked)
        axiosReq.push(tbEform.lockById(item._id.$oid || item._id))
    });
      ...
      return (
        <React.Fragment>
          <div className="main portlet fade-in">
            <BreadCrumbs title={"Danh sách biểu mẫu"} route={[{ label: 'Quản lý biểu mẫu', value: '/quan-ly/eform' }]} />
            <div className="portlet-title">
              <div className="caption">
                <i className="fas fa-grip-vertical" />Danh sách biểu mẫu
              </div>
            ...
            <div className="card">
              <div className="card-header">
                <Link to={'/quan-ly/eform/0'} className="btn btn-sm btn-outline-primary border-radius">
                    ...
                    <tbody>
                      {danhsach.map((item, index) => {
                        return <tr key={index} >
                          ...
                          <td>{item.Ten}</td>
                          <td>{item.Ma}</td>
                          <td>{item.LinhVuc ? item.LinhVuc.Ten : ''}</td>
                          <td>{item.KichHoat ? 'Kích hoạt' : ' '}</td>
                          <td>
                            <Link to={'/quan-ly/eform/' + item._id.$oid || item._id} title="Chi tiết" className="btn btn-sm btn-outline-info border-radius"><i className="fas fa-pencil-alt" /></Link>
