...
class DanhSach extends Component {
  ...
  componentDidMount = async () => {
    this._getDanhSachThongTinEform(this._createFilter())
  }
  ...
  _getDanhSachThongTinEform = async (query) => {
    let data = await tbThongTinEform.getAll(query)
  ...
  _handleDeleteMulti = async () => {
    ...
    danhsach.forEach((item, index) => {
      if (item.checked)
        axiosReq.push(tbThongTinEform.deleteById(item._id.$oid || item._id))
    });
  ...
  _handleLockMulti = async () => {
    ...
    danhsach.forEach((item, index) => {
      if (item.checked)
        axiosReq.push(tbThongTinEform.lockById(item._id.$oid || item._id))
    });
      ...
      return (
        <React.Fragment>
          <div className="main portlet fade-in">
            <BreadCrumbs title={"Danh sách thông tin biểu mẫu"} route={[{ label: 'Quản lý thông tin biểu mẫu', value: '/quan-ly/thong-tin-eform' }]} />
            <div className="portlet-title">
              <div className="caption">
                <i className="fas fa-grip-vertical" />Danh sách thông tin biểu mẫu
              </div>
            </div>
            <div className="card">
              <div className="card-header">
                <Link to={'/quan-ly/thong-tin-eform/0'} className="btn btn-sm btn-outline-primary border-radius">
                    ...
                    <tbody>
                      {danhsach.map((item, index) => {
                        return <tr key={index} >
                          ...
                          <td>
                            <Link to={'/quan-ly/thong-tin-eform/' + item._id.$oid || item._id} title="Chi tiết" className="btn btn-sm btn-outline-info border-radius"><i className="fas fa-pencil-alt" /></Link>
