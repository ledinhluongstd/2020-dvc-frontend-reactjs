...
class ChiTiet extends Component {
  ...
  _init = async () => {
    ...
    if (!this.state.isInsert) {
      let data = await tbThuocTinhDanhMuc.getById(id);
      if (data) {
  ...
  _handleUpdateInfo = async (stay) => {
    ...
    if (isInsert) {
      axiosRes = await tbThuocTinhDanhMuc.create(axiosReq);
    } else {
      let id = this.props.match.params.id;
      axiosRes = await tbThuocTinhDanhMuc.updateById(id, axiosReq);
    }
      ...
      return (
        <div className="main portlet fade-in">
          <BreadCrumbs
            title={'Chi tiết'}
            route={[
              { label: 'Quản lý thuộc tính danh mục', value: '/danh-muc-dtdc/thuoc-tinh-danh-muc' },
              {
                label: 'Thông tin thuộc tính danh mục',
                ...
                <div className="form-body" ref="form">
                  <FormWrapper>
                    <FormInput
                      ...
                      id="Ten"
                      label="Tên"
                      placeholder="Nhập tên thuộc tính danh mục"
                    />
                    <FormInput
                      parentClass="col-md-6"
                      labelClass="col-md-6"
                      ...
                      id="Ma"
                      label="Mã"
                      placeholder="Nhập mã thuộc tính danh mục"
                    />
                  </FormWrapper>
                  <FormWrapper>
                    <FormInput
