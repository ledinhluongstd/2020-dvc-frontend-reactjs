...
class TrichXuat extends Component {
  ...
  _handleLoadDanhMucQuocGia = (inputValue, callback) => {
    ...
    this.state.searchDMTimeOut = setTimeout(async () => {
      ...
      filter.pagesize = 1000;
      filter.count = true
      filter.filter = JSON.stringify({ CategoryName: cmFunction.regexText(inputValue) });
      filter = new URLSearchParams(filter).toString();
      let danhmuc = [], dsDanhMuc = await publicServices.getDanhMucQuocGiaBQP(filter)
      dsDanhMuc = dsDanhMuc && dsDanhMuc._embedded ? dsDanhMuc._embedded : [];
      dsDanhMuc.map(item => {
        if (!item.TotalItem) danhmuc.push(item)
      })
      dsDanhMuc = cmFunction.convertSelectOptions(dsDanhMuc, '_id.$oid', 'CategoryName', danhmuc);
      callback(dsDanhMuc);
    }, 500);
  };

  _handleDanhMucQuocGiaChange = async (value) => {
    // let dulieudanhmucquocgia = await publicServices.getDanhMucQuocGiaBQP('GET', value.UrlGetList)
    this.state.danhmucquocgiaSelected = value;
    this.state.dulieudanhmucquocgia = value.BanGhi // dulieudanhmucquocgia
    this.state.isTrichXuat = false
    this.state.isChecked = false
  ...
  _handleExportDMQG = async () => {
    ...
    let dulieudanhmucquocgia = danhmucquocgiaSelected.BanGhi//await publicServices.getDanhMucQuocGiaBQP('GET', danhmucquocgiaSelected.UrlGetList)
