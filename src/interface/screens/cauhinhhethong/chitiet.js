...
class ChiTiet extends Component {
  ...
  _init = async () => {
    ...
    if (!this.state.isInsert) {
      let data = await tbCauHinhHeThong.getById(id);
      if (data) {
        ...
        this.state.checkIsBoolean = data.KieuDuLieu && (data.KieuDuLieu.Ma === 'boolean')
  ...
  _handleChangeElementCheckBox = (evt) => {
  ...
  _handleKieuDuLieuLoadOptions = (unputValue, callback) => {
    ...
    this.state.kieudulieuTimeout = setTimeout(async () => {
      let kieudulieu = []
      Object.keys(CONSTANTS.KIEU_DU_LIEU_V2).map(function (key, index) {
        kieudulieu.push(CONSTANTS.KIEU_DU_LIEU_V2[`${key}`])
      });
  ...
  _handleKieuDuLieuChange = (sel) => {
    ...
    this.state.checkShowTableOptions = sel && (sel.value === 'select'
      || sel.value === 'radio'
      || sel.value === 'arrays'
      || sel.value === 'checkbox')
    this.state.checkIsBoolean = sel && (sel.value === 'boolean')
    this.forceUpdate()
    // if()
  }
  ...
  _handleUpdateInfo = async (stay) => {
    ...
    if (isInsert) {
      axiosRes = await tbCauHinhHeThong.create(axiosReq);
    } else {
      let id = this.props.match.params.id;
      axiosRes = await tbCauHinhHeThong.updateById(id, axiosReq);
    }
  ...
  render() {
    ...
    let { kieudulieuSelected, modalIsOpen, tbKieudulieu, modalEditIndex, checkShowTableOptions, checkIsBoolean } = this.state
      ...
      return (
        <div className="main portlet fade-in">
          <BreadCrumbs
            title={'Chi tiết'}
            route={[
              { label: 'Quản lý cấu hình hệ thống', value: '/danh-muc-dtdc/thuoc-tinh-danh-muc' },
              {
                label: 'Thông tin cấu hình hệ thống',
                value: '/danh-muc-dtdc/thuoc-tinh-danh-muc/:id',
              },
            ]}
          />
                ...
                <div className="form-body" ref="form">
                  <FormWrapper>
                    <FormInput
                      ...
                      id="Ten"
                      label="Tên"
                      placeholder="Nhập tên cấu hình hệ thống"
                    />
                    <FormInput
                      parentClass="col-md-6"
                      labelClass="col-md-6"
                      ...
                      id="Ma"
                      label="Mã"
                      placeholder="Nhập mã cấu hình hệ thống"
                    />
                  </FormWrapper>
                  <FormWrapper>
                    <FormInput
                  ...
                  {!checkShowTableOptions && <FormWrapper>
                    {checkIsBoolean ? <FormInput type="checkbox"
                      onChange={this._handleChangeElementCheckBox}
                      defaultValue={form.GiaTri || false}
                      id="GiaTri"
                      label="Kích hoạt chính thức" /> : <FormInput
                        parentClass="col-md-6"
                        labelClass="col-md-6"
                        inputClass="col-md-6"
                        required={true}
