...
class DanhSach extends Component {
  ...
  _checkItem = (item) => {
    ...
    if (item.NhomDonVi) { return item.NhomDonVi.Ten } else { return " " }
  }
      ...
      return (
        <React.Fragment>
          <div className="main portlet">
            <BreadCrumbs title={"Danh sách đơn vị"} route={[{ label: 'Quản lý đơn vị', value: '/quan-tri-ket-noi/don-vi' }]} />
            ...
            <div className="card">
              <div className="card-header">
                <Link to={'/quan-tri-ket-noi/don-vi/0'} className="btn btn-sm btn-outline-primary border-radius">
                    ...
                    <tbody>
                      {danhsach.map((item, index) => {
                        return <tr key={index} >
                          ...
                          <td>{item.Ten}</td>
                          <td>{item.NguoiQuanTri}</td>
                          <td>{item.SDT}</td>
                          <td>{item.Email}</td>
                          <td>{item.KichHoat ? 'Kích hoạt' : ' '}</td>
                          {checkSuperAdmin && <td><span style={{ color: 'red' }}>{item.isActive ? '' : 'Đã xóa'}</span></td>}
                          <td>
                            <Link to={'/info-donvi/' + item._id.$oid || item._id} title="Chi tiết" className="btn btn-sm btn-outline-info border-radius"><i className="fas fa-eye" /></Link>
                            <Link to={'/quan-tri-ket-noi/don-vi/' + item._id.$oid || item._id} title="Chỉnh sửa" className="btn btn-sm btn-outline-info border-radius"><i className="fas fa-pencil-alt" /></Link>
